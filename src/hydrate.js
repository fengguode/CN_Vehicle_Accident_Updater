...
const columns = ['fingerprint', 'canonical_url', 'source_url', 'source_name', 'platform', 'external_id', 'title', 'content', 'author', 'published_at', 'collected_at', 'event_date', 'brand', 'model', 'cause', 'cause_confidence', 'adas_mode', 'road_type', 'severity', 'province', 'city', 'injuries', 'fatalities', 'verification_status', 'relevance_score', 'labels_json', 'raw_json', 'duplicate_of', 'review_notes', 'created_at', 'updated_at'];
...
export function hydratePublicData(file, db = openDb()) {
  ...
  try {
    for (const row of rows) {
      ...
      const values = [row.fingerprint, row.canonical_url ?? row.source_url ?? null, row.source_url ?? null, row.source_name || 'public dataset', row.platform ?? null, row.external_id ?? null, row.title, row.content || '', row.author ?? null, row.published_at ?? null, row.collected_at || row.published_at || new Date().toISOString(), row.event_date ?? null, row.brand || 'Unknown', row.model ?? null, row.cause || 'unclassified', Number(row.cause_confidence || 0), row.adas_mode || 'unknown', row.road_type || 'unknown', row.severity || 'unknown', row.province ?? null, row.city ?? null, row.injuries ?? null, row.fatalities ?? null, row.verification_status || 'unverified', Number(row.relevance_score || 0), labelsJson, JSON.stringify({ hydrated_from: 'public-data', id: row.id, fingerprint: row.fingerprint }), row.duplicate_of ?? null, row.review_notes ?? null, row.created_at || row.collected_at || new Date().toISOString(), row.updated_at || row.collected_at || new Date().toISOString()];
