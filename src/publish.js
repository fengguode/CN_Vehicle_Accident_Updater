...
const reports = rows.map((row) => ({
  id: row.id, fingerprint: row.fingerprint, source_url: row.source_url, source_name: row.source_name,
...
const metadata = { schema_version: '1.0.0', generated_at: generatedAt, record_count: reports.length,
...
const cards = reports.map((r) => `<article><time>${esc((r.event_date || r.published_at || '').slice(0, 10))}</time><h2>${esc(r.title)}</h2><p>${esc(r.source_name)} · ${esc(r.brand)} · ${esc(r.cause)} · ${esc(r.verification_status)}</p>${r.source_url ? `<a href="${esc(r.source_url)}" rel="noreferrer">查看来源</a>` : ''}</article>`).join('\n');
