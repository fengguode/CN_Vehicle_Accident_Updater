...
const reports = rows.map((row) => ({
  ...
  platform: row.platform, title: row.title, title_zh: row.title, title_en: row.title_en || englishTitle(row), title_zh_short: abstract(row.title, 140), title_en_short: abstract(row.title_en || englishTitle(row), 140), content: row.content, content_zh: row.content, content_en: row.content_en || row.english_description || englishDescription(row), summary_zh: abstract(`${row.title || ''}. ${row.content || ''}`), summary_en: abstract(row.english_description || englishDescription(row)), author: row.author,
