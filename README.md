...
The frozen v0.1.0 release remains unchanged. The `vnext-home-hosted` branch separates the browser UI, database API, public-news updater, and social collector. The home-hosted HTTPS entry point is [China ADAS Incident Database](https://win-dlf0f69f65u.tail68f4a2.ts.net/); availability depends on this PC, internet, and Tailscale Funnel. Accounts are invitation-only and require administrator approval before report access. See [the step-by-step self-hosting plan](docs/SELF_HOSTED_DATABASE_PLAN.md).

The vNext service runs locally on port 8788 and stores reports, votes, source state, and SVM scores in one SQLite database. It serves a report-free HTML shell whose JavaScript fetches paginated report data from the API. The 294-report/72-vote migration was reconciled and is idempotent. Public-news sources and social JSONL imports use separate commands. Accounts are local to this service: no GitHub, WeChat, or other identity provider is used. Registration requires a one-use invitation issued by a logged-in member. New registrations remain pending until an administrator explicitly approves database access; their signed-in session can be used to change their password, but report, filter, summary, and invitation APIs remain unavailable until approval. An administrator can later revoke approval without logging the user out; the database API blocks immediately and the page refreshes account access state every 30 seconds. The initial administrator is created once from the local terminal:
...
Run `vnext:bootstrap-admin` once, interactively, before starting the service. It refuses non-interactive input, accepts no password argument, hashes the password with scrypt, and refuses to run once an admin exists. Use at least 12 password characters. The administrator can manage user roles, database approval, and account enabled status; only approved accounts can read data or create invitations. Approved members can create invitations that expire after seven days and can be used once. Signed-in users can change their own password; changing it revokes their other sessions. Disabling an account revokes all of its sessions, and only an administrator can restore access. The last approved, active administrator cannot be demoted, unapproved, or disabled. Session tokens are random, stored only as hashes, and sent in HttpOnly/SameSite cookies; the `Secure` attribute is applied for HTTPS requests and omitted for loopback HTTP development. Password hashes and invitation codes are never exposed in list APIs. Keep the home host patched and reachable only through HTTPS when accessed remotely.

`npm run vnext:test-auth` runs a local authentication lifecycle smoke check. It temporarily changes then restores the administrator password, creates and removes one test member and invitation, and verifies registration, approval-gated report/filter/summary/invitation access, admin approval grant/revocation/re-grant, password changes, session revocation, account disable/restore, and last-admin protection. It requires `ADAS_TEST_ADMIN_PASSWORD`; `ADAS_TEST_BASE_URL` and `ADAS_DB_PATH` can point it at a disposable local test service/database. Use an isolated database so the test cannot change real accounts.

Do not publish or commit the vNext SQLite database. `npm run vnext:backup` creates an integrity-checked online backup and restore drill outside this repository. Password recovery is intentionally not public; preserve the administrator credentials and verified backups.

The unified service is connected to Tailscale Funnel. Use `npm run vnext:daily` for one locked public-news/social-inbox update; `npm run vnext:test-io` and `npm run vnext:test-https` are disposable-account acceptance checks. The HTTPS smoke check requires `ADAS_TEST_BASE_URL` set to the Funnel origin. Windows tasks for the service, five-minute watchdog, daily update, and backup are registered by `scripts/install-vnext-tasks.ps1`. On this PC, the service has an AtStartup trigger and the service/watchdog use password-backed background logon under `guofe`; both passed forced-failure recovery tests. The daily and backup tasks still use interactive logon and require sign-in to run. A physical reboot has not been tested. To configure boot tasks on a different PC, an administrator must grant the service user batch-logon permission and run `scripts/configure-vnext-password-boot.ps1`; the Windows password is entered into a local secure prompt and is not saved in the repository.
