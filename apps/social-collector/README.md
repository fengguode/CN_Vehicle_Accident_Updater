...
Run the isolated importer with `npm run vnext:import-social`. It processes only the `manual-platform-exports` inbox source. After a fully successful import, each `.jsonl` input is renamed to `.jsonl.processed` in the same folder so the next daily run reads only new files. The original bytes remain recoverable. The public-feed updater runs with `npm run vnext:update-public` and is configured separately in `public-sources.json`; it does not invoke social-platform search or browser enrichment.
